...
pub fn generate(spec: &[ScSpecEntry]) -> String {
    let spec = spec.iter().map(Entry::from).collect::<Vec<_>>();
    serde_json::to_string_pretty(&spec).expect("serialization of the spec entries should not have any failure cases as all keys are strings and the serialize implementations are derived")
}
